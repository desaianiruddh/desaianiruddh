# 🚀 Hi there, I'm Aniruddh Desai 👋  

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=26&pause=1000&color=00F7FF&width=600&lines=Senior+Software+Engineer;Full+Stack+Developer+(React+%2B+Node);Building+Scalable+Web+Applications;Always+Learning+Something+New..." />

---

## 🧑‍💻 About Me  

💡 Passionate **Full Stack Developer** with 4+ years of experience  
⚛️ Strong in **React, Next.js & modern frontend architecture**  
🛠️ Backend experience with **Node.js, Express & MongoDB**  
🚀 Love building **scalable, reusable & high-performance apps**  
🤝 Collaborative, problem-solver & continuous learner  

---

## 🛠️ Tech Stack  

### 👨‍💻 Frontend  
![React](https://img.shields.io/badge/-React-000?&logo=react)  
![NextJS](https://img.shields.io/badge/-Next.js-000?&logo=next.js)  
![Redux](https://img.shields.io/badge/-Redux-764ABC?&logo=redux)  
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?&logo=typescript)  

### 🎨 UI & Styling  
![SCSS](https://img.shields.io/badge/-SCSS-CC6699?&logo=sass)  
![Ant Design](https://img.shields.io/badge/-AntDesign-0170FE?&logo=ant-design)  
![MUI](https://img.shields.io/badge/-MaterialUI-0081CB?&logo=mui)  

### ⚙️ Backend & Database  
![NodeJS](https://img.shields.io/badge/-Node.js-339933?&logo=node.js)  
![Express](https://img.shields.io/badge/-Express-000000?&logo=express)  
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?&logo=mongodb)  

---

## 📊 GitHub Stats  

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=desaianiruddh&show_icons=true&theme=tokyonight" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=desaianiruddh&theme=tokyonight" />
</p>

---

## 📈 Contribution Graph  

<img src="https://github-readme-activity-graph.vercel.app/graph?username=desaianiruddh&theme=react-dark" />

---

## 🧠 What I Do Best  

✨ Build reusable & scalable UI components  
⚡ Optimize performance & handle complex frontend logic  
🔄 Efficient state management with Redux (Thunk & Saga)  
🧩 API integration & clean architecture design  
📊 Handle large datasets & optimize rendering  

---

## 🏆 Achievements  

🏅 AWS Certified Cloud Practitioner  
📈 Led projects with client collaboration  
🚀 Delivered high-quality scalable solutions  

---

## 🌐 Connect With Me  

📧 desai.ani.ap@gmail.com  
💼 https://www.linkedin.com/in/aniruddh-desai-94380b202  
💻 https://github.com/desaianiruddh  

---

## ⚡ Fun Fact  

💭 *"Clean code + Performance = ❤️"*  
🔥 I love turning complex problems into simple UI  

---

## 🎯 Current Focus  

- 🚀 Full Stack Development  
- ⚡ Performance Optimization  
- 🧠 Advanced React Patterns  

---

⭐️ From https://github.com/desaianiruddh
